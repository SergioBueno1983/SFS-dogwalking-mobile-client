  ...
  const handleAddService = async () => {
    ...
    try {
      ...
      if (!fecha) {
        ...
        setMensaje(
          `El día seleccionado (${dayNames[selectedDay]}) no coincide con los días permitidos del turno (${turn.dias.join(", ")}).`,
        );
        valid = false;
      }
      ...
      if (fecha.getDate() === today.getDate()) {
        ...
        horaFin.setHours(
          parseInt(turn.hora_fin.split(":")[0]),
          parseInt(turn.hora_fin.split(":")[1]),
          0,
          0,
        );
        if (horaFin < new Date()) {
          setMensaje(
            "No puede solicitar un servicio para hoy, porque ya se ha terminado el turno",
          );
          valid = false;
        }
      }
      ...
      if (response.ok) {
        ...
        setServicesRequest((prevPendingServices) => [
          ...prevPendingServices,
          responseData.data,
        ]);
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.infoText}>
        Total: ${turn?.tarifa * cantidadMascotas}
      </Text>
